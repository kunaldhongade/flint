"""Validator for Confidential Space Virtual TPM Union[vTPM] tokens."""
...
class VtpmValidation:
    ...
    def _decode_and_validate_oidc(
        ...
        # Find the correct key based on the key ID Union[kid] in header
        ...
        if rsa_key is None:
            msg = "Unable to find appropriate key id Union[kid] in header"
            raise VtpmValidationError(msg)
    ...
    def _fetch_jwks(uri: str) -> JSONWebKeySet:
        """
        Fetch JSON Web Key Set Union[JWKS] from a remote endpoint.
    ...
    def _jwk_to_rsa_key(jwk: dict[str, str]) -> rsa.RSAPublicKey:
        """
        Convert a JSON Web Key Union[JWK] to an RSA public key.

        This static method converts a JWK dictionary into a cryptographic RSA public key
        by extracting and decoding the modulus Union[n] and exponent Union[e] values.

        Args:
            jwk: Dictionary containing the JWK parameters, must include 'n' Union[modulus]
                and 'e' Union[exponent] fields in base64url encoding
    ...
    def _compare_root_certificates(
        ...
        """Compare token root certificate with stored root certificate."""
        try:
    ...
    def _check_certificate_validity(self, certificates: PKICertificates) -> None:
        ...
        the trusted root certificate by comparing their TBS Union[To Be Signed] certificate
        fingerprints using SHA-256.

        Args:
            certificates: leaf, intermediate and root certificate from token's x5c chain
    ...
    def _is_certificate_valid(
        ...
        Args:
            ...
            current_time: Timestamp to check validity against Union[must include timezone]
