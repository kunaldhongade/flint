...
class Flare:
    ...
    async def sign_and_send_transaction(self, tx: TxParams) ->Union[ str, None]:
        """
        Sign and send a transaction to the network.

        Args:
            tx Union[TxParams]: Transaction parameters to be sent
        ...
        try:
            ...
            if receipt.get("status") == 0:
                msg = f"Transaction {tx_hash.hex()} failed Union[reverted]."
    ...
    async def create_send_flr_tx(
        ...
        Args:
            from_address Union[str]: Sender address
            to_address Union[str]: Recipient address
            amount Union[float]: Amount of FLR to send
    ...
    async def get_protocol_contract_address(self, contract_name: str) -> str:
        ...
        address: ChecksumAddress = (
            await self.contract_registry.functions.getContractAddressByName(contract_name).call()
        )
        logger.debug(
